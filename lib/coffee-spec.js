(function(){
  var CoffeeScript, _CoffeeScript, add_error, assert, child_process, cleanup_stack, compile_and_run_file, current_describe, debug, describe, errors, failed, fs, global_scope, helpers, init, it, path, print_file, puts, readdirRecursive, relativePaths, report, run_file, run_standalone, run_tests, start_time, succeeded, summarize, sys, term, test_exports, tests, unit;
  ...
  compile_and_run_file = function compile_and_run_file(file, temp_path, verbose, cb) {
    var compiled, dir, file_path, input_code, output_file, output_path, prelude, require_path;
    ...
    return child_process.spawn('node', [output_path], function(err, stdout, stderr) {
      if (verbose) {
  ...
  exports.run = function run(dir, options, cb) {
    var do_end, do_run, filter, mkdir, silent, temp_dir, verbose;
    options = options || {};
    ...
    do_run = function do_run() {
      ...
      iterate = function iterate() {
        ...
        if (options.compile) {
          unit = "test files";
          return compile_and_run_file(file, temp_dir, options.verbose, iterate);
        } else {
