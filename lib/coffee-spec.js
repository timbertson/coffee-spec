(function(){
  ...
  VERBOSITY = undefined;
  init = function init(_global, log_level) {
    var errors;
    tests = [];
    errors = [];
    unit = "tests";
  ...
  num_failed_and_passed = function num_failed_and_passed() {
  ...
  summarize = function summarize(num_fail, num_pass) {
  ...
  exports.run_tests = (run_tests = function run_tests(cb) {
  ...
  run_standalone = function run_standalone() {
  ...
  compile_file = function compile_file(file, temp_path) {
    var dir, test;
  ...
  readdirRecursive = function readdirRecursive(base) {
  ...
  relativePaths = function relativePaths(base, paths) {
    var _a, _b, _c, _d, p;
    _a = []; _c = paths;
    for (_b = 0, _d = _c.length; _b < _d; _b++) {
      p = _c[_b];
  ...
  get_log_level = function get_log_level(opts) {
    var log_level;
  ...
  exports.autorun = function autorun(_global, log_level) {
    init(_global, log_level);
    return setTimeout((function() {
      return run_standalone();
    }), 0);
  };
  test_exports.expect = function expect(num_success) {
  ...
  TestCase = function TestCase(describes, name, body) {
  ...
  TestCase.prototype.toString = function toString() {
    if (this.describes) {
  ...
  TestCase.prototype.poll_for_passes = function poll_for_passes(cb) {
    var iterate, tries;
    tries = 0;
  ...
  TestCase.prototype.expect_passes = function expect_passes(num) {
    this.expected_passes = num;
    return this.expected_passes;
  };
  TestCase.prototype.async_pass = function async_pass(desc) {
    var _a;
  ...
  TestCase.prototype.finished_async_testing = function finished_async_testing(continuation, success) {
    var _a, _b, _c, _d, _e, _f, _g, desc, msg;
    ...
    if (this.async_passes.length && (function() {
      _d = []; _f = this.async_passes;
      for (_e = 0, _g = _f.length; _e < _g; _e++) {
        msg = _f[_e];
        (typeof msg !== "undefined" && msg !== null) ? _d.push(msg) : null;
      }
      return _d;
    }).call(this).length > 0) {
      this.error += " Passes received were:";
      _b = this.async_passes;
      for (_a = 0, _c = _b.length; _a < _c; _a++) {
        desc = _b[_a];
        this.error += ("\n - " + (desc));
      }
  ...
  TestCase.prototype.run = function run(cb) {
    this.started = true;
    active_test = this;
    verbose(" - " + this + " ... ");
    try {
      this.body(__bind(this.async_pass, this, []));
  ...
  TestCase.prototype.report = function report() {
    if (this.success) {
      return null;
    }
  ...
  TestFile = function TestFile(dir, file) {
    this.dir = dir;
    this.file = file;
  ...
  TestFile.prototype.compile = function compile(temp_path) {
    ...
    input_code = prelude + fs.readFileSync(this.file_path);
  ...
  TestFile.prototype.run = function run(cb) {
  ...
  TestFile.prototype.report = function report() {
    if (this.stderr) {
  ...
  info = function info() {
    var a;
    a = __slice.call(arguments, 0, arguments.length - 0);
  ...
  exec = function exec(command, args, cb) {
